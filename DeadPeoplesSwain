...
-- Download script
local version = 1.43
local author = "DeadPeople Swain"
local SCRIPT_NAME = "DeadPeople Swain"
local AUTOUPDATE = true
local UPDATE_HOST = "raw.github.com"
local UPDATE_PATH = "/DeadPe0ple/BoL/Scripts/DeadPeopleSwain.lua".."?rand="..math.random(1,10000)
...
if AUTOUPDATE then
	local ServerData = GetWebResult(UPDATE_HOST, "/DeadPe0ple/BoL/Scripts/version/DeadPeopleSwain.version")
	if ServerData then
		...
		if ServerVersion then
			if tonumber(version) < ServerVersion then
				...
				Say("You have got the latest version (v"..ServerVersion..") of DeadPeople Swain by " .. author)
			end
		end
	else
...
local REQUIRED_LIBS = {
	...
	["DeadPeopleLib"] = "https://raw.githubusercontent.com/DeadPe0ple/BoL-Scripts/master/DeadPeopleLib"
}
...
if not FileExist(LIB_PATH .. "DeadPeopleLib.lua") then return Say("You need DeadPeopleLib for this script. Please download this library before running this script.") end

function InitializeVariables()
	serverMessage = GetWebResult(UPDATE_HOST, "/DeadPe0ple/BoL-Scripts/master/News.txt")
	print("<font color=\"#000000\"><b>DeadPeople Chronicles (Latest News):</b></font> <font color=\"#ffaa00\">" .. tostring(serverMessage) .. "</font>")
...
function OnLoad()
	...
	-- DeadPeopleLib Spell initializing
	...
	Say("Succesfully loaded. Enjoy the script. Please remember this a rework.")
...
function Menu()
	...
	Menu = scriptConfig("DeadPeoples Swain by DeadPeople", "DeadPeoplesSwain.cfg")
